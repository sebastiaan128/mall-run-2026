  ...
  return (
    <div className="min-h-screen bg-ink">
      <header className="border-b border-white/20 px-8 py-5 flex items-center justify-between">
        <div>
          <div className="font-display wide text-lg text-white/60">MALL RUN</div>
          <div className="text-xs text-white/50">Beheer, {user?.email}</div>
        </div>
        <button
          onClick={logout}
          className="text-xs font-sans semiwide font-bold text-white/60 hover:text-white"
        >
          Uitloggen
        </button>
      </header>
        ...
        <nav className="flex gap-2 mb-8 border-b border-white/20">
          {TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`px-4 py-3 font-sans semiwide font-bold text-sm border-b-2 -mb-px ${
                tab === t.id
